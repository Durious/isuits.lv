<template>
    <div class="w-full ltr:text-left rtl:text-right">
        <div v-if="modal" class="np flex justify-between items-center px-6 py-4 mb-4 bg-gray-100 border-b rounded-t">
            <p class="font-bold">{{ $t('Shipping Address') }}</p>
...
export default {
    ...
    data() {
        return {
            ...
            keys: [ 'fname','lname','country','state','address','city','zipcode','phone' ]
        }
    },
    watch: {
        show: function() {
            this.form = _.cloneDeep(this.shippingAddress) ?? default_shipping
        }
    },
    methods: {
    submit() {
      this.sending = true
      this.$inertia.put(
        this.route('customer_info.shipping.save', this.customer_id),
        this.form,
        {
          onSuccess: () => {
            this.$emit('close');
