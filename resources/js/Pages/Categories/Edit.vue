<template>
    <div>
        ...
        <trashed-message v-if="category.deleted_at" class="mb-6" @restore="() => (restore = true)">
            <span>{{ $t('already_deleted_x', { x: $tc('Category') }) }}</span>
        </trashed-message>
        <div class="bg-white rounded shadow max-w-3xl">
            <form @submit.prevent="submit" autocomplete="off">
                ...
                <div class="px-8 pb-8 ltr:-mr-6 rtl:-ml-6 ltr:ml-0 rtl:mr-0 -mb-8 flex flex-wrap">
                    <file-input type="file" accept="image/*" :label="$t('Photo')" v-model="form.photo"
                        :error="errors?.photo" class="ltr:pr-6 rtl:pl-6 pb-8 w-full lg:w-full" />
                </div>
                <div class="px-8 pb-8 ltr:-mr-6 rtl:-ml-6 ltr:ml-0 rtl:mr-0 -mb-8 flex flex-wrap">
...
export default {
    layout: Layout,
    remember: 'form',
    props: { category: Object, icustomFields: Object },
    mixins: [DeleteMixin('category')],
    ...
    data() {
        return {
            ...
            form: {
                ...
                customFields: this.category.customFields.map(item => ({value: item.value, label: item.label})),
            },
            customFields: []
        };
    },
