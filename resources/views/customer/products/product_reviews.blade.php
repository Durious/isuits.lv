<div class="product-reviews" style='position: relative'>
    <div class="slider-area owl-carousel product-comments">
        @foreach ($product->product_reviews as $item)
            ...
            <div class="d-flex">
                ...
                <div class="pl-4">
                    ...
                    <div class="review-ratings">
                        <span class="badge badge-warning mr-3">{{ number_format((float) $item->rate, 1, '.', '') }}</span>
                        @php
                        $full_stars = floor($item->rate);
                        $half_stars = round($item->rate - $full_stars, 1) >= 0.5 ? 1 : 0;
                        @endphp
                        @for ($i = 0; $i < $full_stars; $i++) <i class="fa fa-star"></i>
                            @endfor
                            @if ($half_stars)
                            <i class="fa fa-star-half-o"></i>
                            @endif
                            @for ($i = 0; $i < 5 - $full_stars - $half_stars; $i++) <i class="fa fa-star diff-star"></i>
                                @endfor
                    </div>
